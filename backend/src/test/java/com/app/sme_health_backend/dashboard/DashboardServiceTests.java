...
class DashboardServiceTests {
    ...
    void shouldThrowWhenUserProfileNotFound() {
        ...
        verifyNoInteractions(scoringService, cashFlowService, insightService, recommendationService);
    }
    ...
    void shouldReturnFullyPopulatedDashboard() {
        ...
        when(scoringService.getLatestScore(userId)).thenReturn(Optional.of(score));
        when(insightService.getInsights(userId)).thenReturn(insights);
        when(recommendationService.getRecommendations(userId)).thenReturn(recs);
    ...
    void shouldHandleNewUserWithNoFinancialHistory() {
        ...
        verifyNoInteractions(insightService, recommendationService);
    }
    ...
    void shouldGenerateAndReturnTopInsightAndRecommendationWhenInitiallyAbsent() {
        ...
        when(cashFlowService.getTrendProjection(userId)).thenReturn(CashFlowProjectionResponse.insufficientData(0));
        when(scoringService.getLatestScore(userId)).thenReturn(Optional.of(score));
        when(insightService.getInsights(userId)).thenReturn(List.of(freshlyGeneratedInsight));
        when(recommendationService.getRecommendations(userId)).thenReturn(List.of(freshlyGeneratedRec));
        ...
        verify(insightService).getInsights(userId);
    ...
    void shouldAlwaysUseLatestScoreResult() {
        ...
        when(cashFlowService.getCashFlowHistory(userId)).thenReturn(Collections.emptyList());
        when(cashFlowService.getTrendProjection(userId)).thenReturn(CashFlowProjectionResponse.insufficientData(0));
        when(scoringService.getLatestScore(userId)).thenReturn(Optional.of(latestScore));
        when(insightService.getInsights(userId)).thenReturn(Collections.emptyList());
        ...
        verify(scoringService).getLatestScore(userId);
    }
    ...
    void shouldEnsureTopInsightAndRecommendationCorrespondToLatestScore() {
        ...
        when(cashFlowService.getTrendProjection(userId)).thenReturn(CashFlowProjectionResponse.insufficientData(0));
        when(scoringService.getLatestScore(userId)).thenReturn(Optional.of(latestScore));
        when(insightService.getInsights(userId)).thenReturn(List.of(insight));
        when(recommendationService.getRecommendations(userId)).thenReturn(List.of(rec));
    ...
    void shouldSelectHighPriorityItemOverLowerPriorityEvenIfNotFirst() {
        ...
        when(cashFlowService.getTrendProjection(userId)).thenReturn(CashFlowProjectionResponse.insufficientData(0));
        when(scoringService.getLatestScore(userId)).thenReturn(Optional.of(score));
        when(insightService.getInsights(userId)).thenReturn(List.of(lowInsight, highInsight));
        when(recommendationService.getRecommendations(userId)).thenReturn(List.of(lowRec, highRec));
